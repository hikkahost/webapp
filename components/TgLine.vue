...
<template>
    <component :is="component" v-bind="attributes" class="w-full group flex select-none bg-transparent" :class="pt">
        <div class="flex-grow flex items-center px-3 py-1" :class="[border]">
            <div class="flex flex-row justify-center items-start w-full gap-4">
                ...
                <p v-if="props.description" class="tg-hint leading-none text-xs"
                    :class="[props.subtitle ? 'pt-1.5' : 'pt-2', lineClamp]">
                    {{ props.description }}
                </p>
